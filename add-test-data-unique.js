...
async function addTestData() {
  ...
  try {
    ...
    const [maxSerial] = await connection.execute(
      'SELECT MAX(serial) as max_serial FROM certificates'
    );
    const nextSerial = (maxSerial.max_serial || 1000) + 1;
    console.log(`   📊 Next available serial: ${nextSerial}`);
    
    // Add test certificates with unique serials
    const certificates = [
      {
        id: `c-${Date.now()}-1`,
        serial: nextSerial,
    ...
    // Add test meetings with shorter IDs
    const meetings = [
      {
        id: `m-${Date.now()}-1`,
        teacher_id: teacherId,
