...
async function addTestData() {
  ...
  try {
    ...
    // Get existing teacher or create a new one
    ...
    // Add test assignments with shorter IDs
    const assignments = [
      {
        id: `a-${Date.now()}-1`,
    ...
    for (const assignment of assignments) {
      await connection.execute(
        ...
        [`at-${Date.now()}-${Math.floor(Math.random() * 1000)}`, assignment.id, studentId]
      );
    }
    ...
    // Use a very high serial number to avoid conflicts
    const highSerial = 999999 + Math.floor(Math.random() * 100000);
    console.log(`   📊 Using high serial: ${highSerial}`);
    
    // Add test certificates with high serials
    const certificates = [
      {
        id: `c-${Date.now()}-1`,
        serial: highSerial,
    ...
    // Add test meetings without description field
    const meetings = [
      {
        id: `m-${Date.now()}-1`,
        teacher_id: teacherId,
    ...
    for (const meeting of meetings) {
      await connection.execute(
        'INSERT INTO meetings (id, teacher_id, provider, title, scheduled_at, duration_minutes, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [meeting.id, meeting.teacher_id, meeting.provider, meeting.title, meeting.scheduled_at, meeting.duration_minutes, meeting.status]
      );
    ...
    // Add test materials with file_url field
    const materials = [
      {
        id: `mat-${Date.now()}-1`,
        teacher_id: teacherId,
        title: 'دليل التجويد الأساسي',
        file_url: '/materials/tajweed-basic.pdf',
      ...
      },
      {
        ...
        file_url: '/materials/nun-rules.pdf',
      ...
      },
      {
        ...
        file_url: '/materials/reading-rules.pdf',
    ...
    for (const material of materials) {
      await connection.execute(
        'INSERT INTO materials (id, teacher_id, title, file_url, created_at) VALUES (?, ?, ?, ?, ?)',
        [material.id, material.teacher_id, material.title, material.file_url, material.created_at]
      );
    ...
    console.log('\n🎉 Test data successfully added for student@test.com!');
    console.log('   You can now log in as student@test.com and see real data in the dashboard.');
